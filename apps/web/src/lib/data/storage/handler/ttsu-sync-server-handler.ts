...
import { database, ttsuSyncServerStorageSource$ } from '$lib/data/store';
import type { StorageKey } from '../storage-types';
import { StorageTtsuAuthManager } from '../storage-ttsu-auth-manager';
...
export class TtsuSyncServerStorageHandler extends ApiStorageHandler {
  private defaultServerRoot = 'http://127.0.0.1:8080';

  private ttsuAuthManager: StorageTtsuAuthManager;

  constructor(storageType: StorageKey, window: Window) {
    const ttsuAuthManager = new StorageTtsuAuthManager();

    super(storageType, window, ttsuAuthManager);
    this.ttsuAuthManager = ttsuAuthManager;
  }

  private async reqAny<T>(
    path: string,
    ...
    const serverRoot = (
      (await this.ttsuAuthManager.getServerUrl(this.storageSourceName, this.askForStorageUnlock)) ||
  ...
  protected setInternalSettings(storageSourceName: string): void {
    const newStorageSource = storageSourceName || ttsuSyncServerStorageSource$.getValue();
